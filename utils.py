...
def fetch_poster(movie_id):
    ...
    try:
        ...
        response.raise_for_status() # вызов HTTPError при неудаче
        data = response.json()
        poster_path = data.get('poster_path') # Используем .get(), чтобы избежать ошибки KeyError, если 'poster_path' отсутствует
        if poster_path:
